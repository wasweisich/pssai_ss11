...
public class GraspConstructionHeuristic implements IConstructionHeuristics<TTPInstance, TTPSolution> {
    ...
    private static Logger logger = Logger.getLogger(GraspConstructionHeuristic.class);
    ...
    public VirtualScheduleConstructionMethod getMethod() {
        return method;
    }

    public void setMethod(VirtualScheduleConstructionMethod method) {
        this.method = method;
    }

    @Override
    public GraspConstructionHeuristic clone() throws CloneNotSupportedException {
        GraspConstructionHeuristic clone = (GraspConstructionHeuristic) super.clone();

        if (problemInstance != null)
            clone.problemInstance = problemInstance.clone();

        if (tabuList != null)
            clone.tabuList = tabuList.clone();

        if (virtualSchedule != null) {
            clone.virtualSchedule = new int[virtualSchedule.length][];
            for (int i = 0; i < virtualSchedule.length; i++)
                clone.virtualSchedule[i] = virtualSchedule[i].clone();
        }

        if (virtualValues != null) {
            clone.virtualValues = new TreeSet<IntTripple>();
            for (IntTripple virtualValue : virtualValues) {
                clone.virtualValues.add(virtualValue.clone());
            }
        }

        if (realDistances != null) {
            clone.realDistances = new ArrayList<SortedSet<IntTripple>>();

            for (SortedSet<IntTripple> realDistance : realDistances) {
                SortedSet<IntTripple> clonedRealDistance = new TreeSet<IntTripple>();

                for (IntTripple intTripple : realDistance)
                    clonedRealDistance.add(intTripple.clone());

                clone.realDistances.add(clonedRealDistance);
            }
        }
    ...
    private void createNewVirtualSchedule() {
        ...
        // create matrix
        int[][] matrix = new int[problemInstance.getNoTeams()][problemInstance.getNoTeams()];
        int lastVisited = 0;
        ...
        for (int i = 0; i < problemInstance.getNoTeams(); i++) {
            TreeSet<IntTripple> tmpSS = new TreeSet<IntTripple>(new Comparator<IntTripple>() {
    ...
    private int[][] greekMethod() {
        return TtpRandomConstructionHeuristic.genSchedule(problemInstance.getNoTeams(), problemInstance.getNoRounds());
    }
    ...
    private int[][] getGraspSol() {
        ...
        TreeSet<IntTripple> virtualValuesCopy = new TreeSet<IntTripple>(virtualValues);
        ...
        while (!virtualValuesCopy.isEmpty()) {
            ...
            IntTripple choosenOne = candidateList.get(rand.nextInt(candidateList.size()));
